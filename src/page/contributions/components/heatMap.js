...
export default function HeatMap(props) {
  ...
  useEffect(() => {
    ...
    setMonthValues(v=>v.concat(monValues));
    return () => {};
  }, [data]);
  ...
  const IsMobile = () => {
    return (
      <div className="svg-content">
        <ReactTooltip />
        <span>
          Your contribution in {monthMap[new Date().getMonth() + 1]}{" "}
          <i className="nes-icon is-medum like"></i>
        </span>
  ...
  const IsPC = () => {
    return (
      <div className="svg-content">
        <ReactTooltip />
        <span>
          Your contribution in {data.year}{" "}
          <i className="nes-icon is-medum like"></i>
        </span>
        <CalendarHeatmap
